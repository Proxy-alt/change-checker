...
describe('FolderChangeChecker', () => {
  ...
  describe('with wasm backend', () => {
    let checker;

    before(async () => {
      checker = new FolderChangeChecker({ mtimePath, hashPath, backend: 'wasm' });
      await checker.ready;
    });
    ...
    it('checkFolder', async () => {
      // Clean JSON files to avoid them being counted
  ...
  // Similar tests for other backends could be added, but for brevity, just test one
});
